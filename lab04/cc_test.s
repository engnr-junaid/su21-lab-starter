...
simple_fn:
    #mv a0, t0 removed this line because it is unnecessary 
    li a0, 1
    ret
...
naive_pow:
    # BEGIN PROLOGUE
    addi sp, sp, -4
    sw s0, 0(sp)	# store the value of saved register on stack
    # END PROLOGUE
    li s0, 1
naive_pow_loop:
...
naive_pow_end:
    mv a0, s0
    # BEGIN EPILOGUE
    lw s0, 0(sp)	# load the value of s0 from stack before returning
    addi sp, sp, 4
    # END EPILOGUE
    ret
...
inc_arr:
    ...
    addi sp, sp, -12
    sw ra, 0(sp)
    sw s0, 4(sp)	# store the values of saved registers on stack before using 
    sw s1, 8(sp)
    # END PROLOGUE
...
inc_arr_end:
    # BEGIN EPILOGUE
    lw ra, 0(sp)
    lw s0, 4(sp)	# load the values of saved registers from stack before returnning
    lw s1, 8(sp)
    addi sp, sp, 12
    # END EPILOGUE
    ret
...
helper_fn:
    # BEGIN PROLOGUE
    addi sp, sp, -4   
    sw s0, 0(sp)	# store the value of s0 on stack to avoid overwriting
    # END PROLOGUE
    lw t1, 0(a0)
    addi s0, t1, 1
    sw s0, 0(a0)
    # BEGIN EPILOGUE
    lw s0, 0(sp)	# load the value of s0 back from stack before returning
    addi sp, sp, 4
    # END EPILOGUE
    ret
